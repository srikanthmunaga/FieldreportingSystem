...
public final class Multis extends HttpServlet {
...
//					ps=con.prepareStatement("select unit_code from unit_msrt where ")
				    ps1= con.prepareStatement("insert into INDENT(SEQID,S_ID,S_NAME,CLO_STOCK,NEW_STOCK,BSFLUNIT_UCODE,BSFLUNIT_NAME,IN_BY,PRE_BY,DATE_OF_INDENT)VALUES(INDENTSEQUENCE.nextval,?,?,?,?,?,?,?,?,to_date(?,'dd-mm-yyyy'))");
					for(int i=0; i<staid.length; i++){
