...
public class UnitReports {
	...
	public void FRSReports(HttpServletRequest request, HttpServletResponse response,String fdate,String tdate,String format,String report_name,String bsflunit_ucode1[])
	{
		...
		String bsflunit_ucode[] = bsflunit_ucode1;//request.getParameterValues("emp_id");// System.out.println("hey parameter from on submit ="+emp_id);
		if (bsflunit_ucode == null)
			break jmp;// breaks the jmp: block and sends
						// controle to }//try stmt
		String unitlist = "";
		if (bsflunit_ucode != null && bsflunit_ucode.length != 0) {
			for (int i = 0; i < bsflunit_ucode.length; i++)
				unitlist = unitlist + "'" + bsflunit_ucode[i] + "'"
						+ ",";// data
								// =rs1.getString(1)+":"+rs1.getString(2)+":";
		}
		unitlist = unitlist
				.substring(0, unitlist.length() - 1);
		//System.out.println(emplist);
		parameters.put("bsflunit_ucode", unitlist);// //putting
											// emplist in
											// emp_id
											// parameter
