...
public final class MultiIndnet extends HttpServlet {
...
//					ps=con.prepareStatement("select unit_code from unit_msrt where ")
				    ps1= con.prepareStatement("insert into unit_ind(BSFLUNIT_UCODE,BSFLUNIT_NAME,S_ID,S_NAME,DATE_OF_INDENT,CLOS_STOCK,NEW_STOCK,IN_BY,PRE_BY) VALUES(?,?,?,?,TO_DATE(?,'DD-MM-YYYY'),?,?,?,?)");
					for(int i=0; i<sno.length; i++){
