...
public final class StockUpdate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
					...
					// request).getSession().getAttribute("user") != null)
			{
                ...
                String date_of_stock=request.getParameter("date_os").trim();
                String no_stock=request.getParameter("no_stock");
                String value_of_purchase=request.getParameter("v_purchase");
			...
			//	comp_id = comp_id.substring(comp_id.lastIndexOf("-")+1);
				...
				try {// System.out.println("hey inside the try and going to check the availability of the comp_id");
					...
					if (x == 0) {
						ps1 = con
								.prepareStatement("update STOCK_ENTRY set s_name=?,date_of_stock=to_date(?,'dd-mm-rrrr'),no_of_stock=?,value_of_purchase=? where s_id=?");
						//ps1.setString(1, comp_id.substring(0, 4));
						ps1.setString(1, stat_name);
						ps1.setString(2, date_of_stock);
						ps1.setString(3, no_stock);
						ps1.setString(4, value_of_purchase);
					    ps1.setString(5, sta_id);
...
//						ps1.setString(9, area_id);
						...
						f = ps1.executeUpdate();
						if (f != 0)
							out.println("OKStock master Updated successfully");
						else
							out.println("stock master is not Updated for some reasons");
					}// if((x==0)
					else
