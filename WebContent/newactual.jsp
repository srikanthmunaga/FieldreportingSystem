...
<title>State Head Actual Plan</title>
<% 
...
<%//getting BSFLUNIT_UCODE to assign in BSFLUNIT_UCODE field automatically in enable() function
  ...
  String sql="select huma_id,name,state from frs_user where username='"+username+"'";
  //String sql="select LOWER(BSFLUNIT_NAME) from BSFLUNIT_MSTR where BSFLUNIT_NAME='"+username+"'";
  //System.out.println();
  				String huma_id=null;
  				String areaid=null;
  				String region=null;
  				String name_of_statehead=null;
  				String ucode=null;
				...
				name_of_statehead=svrs.getString(2);
				System.out.println("Name of State Head is "+name_of_statehead);
				region=svrs.getString(3);
				System.out.println("Name of Region is "+region);
				}//BSFLUNIT_UCODE
...
{
//alert("Inside showEmps1()");
/* var cat = document.getElementById("act_id").length;
alert("Cat length = "+cat); */
 		...
 		//	alert("came to"+value);		
			...
			sdrcount[j].style.visibility='visible';
	 		...
	 		//	alert("came to"+value);		
				...
				alert("Unit Visited Hidden");
				var addbtn=document.getElementsByName('addButton');
				addbtn[j].style.visibility='hidden';
				alert("Add Btn Hidden");
				var delbtn=document.getElementsByName('removeButton');
				delbtn[j].style.visibility='hidden';
				alert("Remove Btn Hidden");
				//alert("the text box is"+'textbox'+(j+1));
				
				alert(i);
				alert('textbox'+i);
				alert("the value of j is"+j);
				//alert(document.getElementsByName('textbox'+i).index);
				
				//var m=0;
				//var tt=document.getElementsByName('textbox'+(m+1));
				//var tt=document.getElementsByName('textbox'+(j+1));
				var tt=document.getElementsByName('textbox'+(1));
				alert(tt[j]);
				tt[j].style.visibility='hidden';
			//	m++;
			//	alert(m);
				var namevisited=document.getElementsByName('visited_with');
				namevisited[j].style.visibility='hidden';	
		        ...
		        var custcount=document.getElementsByName('war_cust_count');
				custcount[j].style.visibility='hidden';
				alert("War_Cust_count Hidden");
				var sdrcount=document.getElementsByName('war_sdrcust_count');
				sdrcount[j].style.visibility='hidden';
				alert("War_sdr_count Hidden");
				var remarks=document.getElementsByName('observations');
				//alert("came here");
...
var ops_activitysl4 = document.getElementsByName('observations');//alert("hety got the ops_activitysl again="+ops_activitysl4);
 var n=ops_activitysl4.length;
 j=i-1;
 emp2=emp;

var unit_visited = document.getElementsByName('unit_visited');//alert("hety got the fpo_id="+fpo_id);
		 var visited_with = document.getElementsByName('visited_with');//alert("hety got the fpo_id="+fpo_id);
		addSuggestion2('NAME','visited_with',unit_visited[j].value);
...
// 	alert("the activity value is "+activity);
		if((activity!='') && (activity=='Unit Visit')){ //FieldVisit
	//alert("came to");		
		var unitvisited=document.getElementsByName('unit_visited');
		unitvisited[j].style.visibility='visible';
		//alert(j);
		...
		//below two lines Added by Rajashekhar just to delete village
		var textboxAssign2 = document.getElementsByName('textbox'+(j+1));
		delAllVillageFields(j+1,textboxAssign2.length);//alert("deleted previous rows::"+'textbox'+(j+1));
		...
		var sdrcount=document.getElementsByName('war_sdrcust_count');
		sdrcount[j].style.visibility='hidden';
		var obs=document.getElementsByName('observations');
		obs[j].style.visibility='hidden';
	}
		else if(activity=='Field Visit'){ //FieldVisit
			//	alert("came to"+value);		
			var unitvisited=document.getElementsByName('unit_visited');
			unitvisited[j].style.visibility='visible';
			//alert(j);
			...
			var delbtn=document.getElementsByName('removeButton');
			delbtn[j].style.visibility='visible';
			//Added by Rajashekhar to add one Village field
			addVillageField(j+1,1);
			var namevisited=document.getElementsByName('visited_with');
			namevisited[j].style.visibility='visible';
			var custcount=document.getElementsByName('war_cust_count');
			custcount[j].style.visibility='visible';
			...
			sdrcount[j].style.visibility='visible';
			var obs=document.getElementsByName('observations');
			obs[j].style.visibility='visible';
		}
		else{
...
<script type="text/javascript" language="javascript">
	function generateTable(war_period) {// this funciton just used to get the row count of the table
		...
		//date comparision code   
			   ...
			   var ldate=(document.form1.war_fdate.value).split("-");
		       var ldate = new Date(ldate[2], ldate[1]-1, ldate[0]); //alert("hey to rday date is ="+prdate);
			   if(ldate > sysdate)
		     	{
				 alert("selected From Date should not more than the System date");
				 document.form1.war_fdate.focus();
				 return false; 
			     }
			...
			//date comparision code   
				   ...
				   var ldate=(document.form1.war_tdate.value).split("-");
			       var ldate = new Date(ldate[2], ldate[1]-1, ldate[0]); //alert("hey to rday date is ="+prdate);
				   if(ldate > sysdate)
			     	{
					 alert("selected To Date should not more than the System date");
					 document.form1.war_tdate.focus();
					 return false; 
				     }
			...
			} else if (star3[0].substr(0, 2) == 'NO')//if(star3=='')//if(star3.length<=3)//because here 1st 3 items of validation self empid,and existance of plan ,dates are in the selected weekoff or not
			{
				...
				for ( var i = 0; i < star3.length; i++)
					...
					if(i!=star3.length-1)
					$(".addRow").trigger('click');//addTable('tableID');
				//	document.form1.unit_visited[i].disabled=true;
					//document.form1.textbox[i].disabled=true;
			//		document.form1.visited_with[i].disabled=true;
				//	document.form1.war_cust_count[i].disabled=true;
					//document.form1.war_sdrcust_count[i].disabled=true;
					}
...
{//alert("hey inside the generate button enable");
 ...
 { //alert("hey inside the if: means the huma_id is !=''");
  ...
  if(document.form1.name_sh.value=='')
  {
	  alert("please enter the Name of State Head");
	  document.from1.name_sh.focus();
	  return false;
  }
...
//validation for the dynamic payment schedule table
	...
	var unit_visited=document.getElementsByName('unit_visited');
	var visited_with=document.getElementsByName('visited_with');
...
//1. Check Activity selected or not
//2. Name of the unit visited
//3. Name Of The LSR/UH Visited with
//4. No.of OD/FTOD Customers met
//5. No.of Customers Met
//6. Validate Remarks, if only data is there just check like char ////////:::::::: 

//---------------------------Villages validation starts here dont touch.
	...
	//war_cust_count validation start here	 
		...
		//   if(war_cust_count[k].style.visibility=='visible')
		 //  {
		   ...
		   if((war_cust_count[k].style.visibility=='visible') && (war_cust_count[k].value=='')){
		    ...
		    //{// alert("hey the inside ops_place");
			 alert("Please enter the No.of FTOD/OD Customers to be visited correctly"); 
...
//war_sdrcust_count validation start here	 
		   ...
		   if((war_sdrcust_count[k].style.visibility=='visible')&&(war_sdrcust_count[k].value==""))
		   {//alert("the number after the point="+war_sdrcust_count[k].value.split('.')[1].length);
		    ...
		    //{ //alert("hey the inside ops_place");
			 alert("Please enter No.of Customers met correctly"); 
		   ...
		   if((unit_visited[k].style.visibility=='visible')&&(unit_visited[k].value=="")){
			   alert("please enter the value of unit visited");
			   unit_visited[k].focus();
			   return false;
		   }
		if((visited_with[k].style.visibility=='visible')&&(visited_with[k].value=="")){
			   alert("please enter the name of unit visited with");
			   visited_with[k].focus();
			   return false;
		   }
...
/*
function jschange(){
	          alert("Inside the jschange()");
	          var NoOfRow = document.getElementsByName('activities'); 
	  		  var NoOfRows = NoOfRow.length;
	  		  alert("NoOfRows = "+NoOfRows);
	         // alert("Length = "+len);
	         for(var i=0;i<NoOfRows;i++)
	        	 {
	         var val = document.getElementById("activities")[i].value; 
	         alert("Value = "+val);
	         if(val == "FV")
	        	 {
	        	 alert("FV Select");
	        	 document.form1.unit_visited[i].disabled=false;
	        	 document.form1.textbox1[i].disabled=false;
	        	 document.form1.visited_with[i].disabled=false;
	        	 document.form1.war_cust_count[i].disabled=false;
	        	 document.form1.war_sdrcust_count[i].disabled=false;
	        	 }
	         else{
	        	 alert("FV not select");
	         }
		...
		$("#form1").submit(function(){sessioncheck();//alert("hey inside the submit function");
		   if(document.form1.onSubmit==validateForm()) 
			...
			if(document.form1.save.style.display=='inline'){
			  ...
			  var program='ActualNew';	}
			else  
				
			   var program='UpdateActualnew'; //alert("hey the program is="+program);	
				$.ajax({ 
				   	url: program,
		          	data: allElements,
		          	type: 'post',
...
//--------------------------------------------------------------------------------recent Working area start	
	...
	//$(".delDynTextBox").trigger('click');	
		var date = document.getElementsByName('war_date'); 
		var rowindexLast = date.length; //alert("rowindexLast="+rowindexLast);
...
//---------------add function for inner dynamic filds 
		...
		var par=this.parentNode; 
 		...
 		var s=rowindex-1;
 		var unit_visited=document.getElementsByName('unit_visited');
            //alert(rowindex+"whilw adding");
	 	($(this).parent()).append('<input type="text" name="textbox'+rowindex+'" id="textbox'+rowindex+'" size="18" onkeyup="allowupdate();" style="border-color:#0099FF;" maxlength="65"  onfocus="addSuggestion2(\'VCODE\',\'textbox'+rowindex+'\',unit_visited['+s+'].value);">');
	 	//counter++;
	     });
	    // alert(rowindex);
...
function addVillageField(rowindex,count)
	{
		...
		var unit_visited=document.getElementsByName('unit_visited');
			//alert(rowindex);
		($(btnObj).parent()).append('<input type="text" name="textbox'+rowindex+'" id="textbox'+rowindex+'" size="18" onkeyup="allowupdate();" maxlength="65" style="border-color:#0099FF;" onfocus="addSuggestion2(\'VCODE\',\'textbox'+rowindex+'\',unit_visited['+s+'].value);">');
		//alert("the textbox"+rowindex);
...
function delVillageField(rowindex,count)
	{
		...
		{
	     //alert("'textbox'+rowindex=textbox"+rowindex);
	      /*This below line changed Rajashekhar to remove all dynamic fields to zero*/
	     if(len-1-r<0)
	     //if(len-r<0)
	    	 break;
	     else
	     $(textbox[len-1-r]).remove();
	     //$(textbox[len-r]).remove();
		}//alert("deleted all fields in delVillageField function");
	    addVillageField(rowindex, 1);//adding one if the only first dynamic field deleted
		//alert("end of del VillageField()");
	}
function delAllVillageFields(rowindex,count)
{
	//alert("inside delVillageField() and rowIndex="+rowindex);alert("count="+count);
	var textbox = document.getElementsByName('textbox'+rowindex);
    var len = textbox.length;
...
for(var x5=1; x5<l; x5++)
 ...
 document.getElementById("name_sh").value =star[x][1];
 document.getElementById("reg").value =star[x][2];
 ...
 var war_cust_count = document.getElementsByName('war_cust_count');
 var war_sdrcust_count = document.getElementsByName('war_sdrcust_count');
 var war_withuh=document.getElementsByName('visited_with');
 var war_stay1=document.getElementsByName('unit_visited');
 var war_obs=document.getElementsByName('observations');
 //var war_with=document.getElementsByName('with_uh');//var ops_narration = document.getElementsByName('ops_narration');
// var war_sdrcust_count_opted = document.getElementsByName('war_sdrcust_count_opted');
 var war_villages = document.getElementsByName('war_villages');
 ...
 //alert("hinsiede the for loop,and the  head="+star[k][2]+"--");
  //alert("the activitsl_id="+star[k][5]);
   activity[j].value=star[k][3];
   //alert("the value is  "+star[k][3]);
 if(star[k][3]=="AC2-Field Visit") {
	 if(k>x+c2[x1])
		   break; 
	 //alert("in field visit");
   war_date[j].value=star[k][5];//alert("the war_date_id="+star[k][6]);
  if(j==0)
	  document.getElementById("war_fdate").value=star[k][5];
  if(j==c2[x1]-1)
	  document.getElementById("war_tdate").value=star[k][5];
  war_day[j].value=star[k][12];
  // village1[j].value=star[k][7];
  war_cust_count[j].value=star[k][8]; 
  war_sdrcust_count[j].value=star[k][9];
  war_withuh[j].value=star[k][6];
  war_stay1[j].value=star[k][7];
  war_obs[j].value=star[k][10];
  var addbtn=document.getElementsByName('addButton');
	addbtn[j].style.visibility='visible';
  ...
  war_stay1[j].style.visibility='visible';
  war_cust_count[j].style.visibility='visible'; 
  war_sdrcust_count[j].style.visibliity='visible';
  war_withuh[j].style.visibility='visible';
  
  //war_sdrcust_count_opted[j].value=star[k][7];
  //alert("star[k][8]="+star[k][8])
  var villages=star[k][11].split("::");
 ...
 // textboxAssign[m].style.visibility = 'visible';
  ...
  war_day[j].value=star[k][12];
  // village1[j].value=star[k][7];
  war_cust_count[j].style.visibility='hidden';
  war_cust_count[j].value='';
  war_sdrcust_count[j].style.visibility='hidden';
  war_sdrcust_count[j].value='';
  war_withuh[j].style.visibility='hidden';
  war_withuh[j].value='';
  war_stay1[j].style.visibility='hidden';
  war_stay1[j].value='';
  var addbtn=document.getElementsByName('addButton');
	addbtn[j].style.visibility='hidden';
	...
	var villages=star[k][11].split("::");
	  ...
	  for(var m=0; m<textBoxLen; m++)
	   {  
	  //alert("First village value is :"+villages[m]);
	  delVillageField(j+1,textboxAssign2.length);
	   textboxAssign[m].style.visibility ='hidden'; 
	...
	// addVillageField(j+1,textBoxLen-1); //alert("added new village fields");
  //var textboxAssign2 = document.getElementsByName('textbox'+(j+1));
  //delVillageField(j+1,textboxAssign2.length+1);
...
//alert("insede table");
	...
	// document.form1.BSFLUNIT_UCODE.focus();//To focus at field cont_id at the time of query
	 ...
	 document.form1.huma_id.value="<%=huma_id%>";
	 <%-- document.from1.name_sh.value="<%=name_of_statehead%>";
	 document.from1.reg.value="<%=region%>"; --%>
	 
	//--------------------------------------Common (buttons) code
//alert("came here to check");
...
//document.form1.ops_sno.value=1;//newly added for dynamic table
 ...
 document.form1.reg.value="<%=region%>";
 document.form1.huma_id.value="<%=huma_id%>";
 document.form1.name_sh.value="<%=name_of_statehead%>";
...
//Done By Rajesh
//document.form1.unit_visited.disabled=true;
//document.form1.textbox1.disabled=true;
//document.form1.visited_with.disabled=true;
//document.form1.war_cust_count.disabled=true;
//document.form1.war_sdrcust_count.disabled=true;
...
<p align="right">
 <table width="1150" align="center">
  ...
  <fieldset style="background-color:">
	<legend class="formTitle"><b> State Head Actual Plan Monthly</b> </legend>
    ...
    <tr>
      ...
      <td width="" colspan="2" align="left" style=""><div align="left"><input name="BSFLUNIT_UCODE" id="BSFLUNIT_UCODE" type="hidden" size="30" maxlength="50" style="border-color:#0099FF;" onfocus="addSuggestion('BSFLUNIT_UCODE_roles','BSFLUNIT_UCODE');" onblur="showEmp('BSFLUNIT_NAME.area_name',BSFLUNIT_UCODE.value,'');"/><input type="hidden" name="UHLOG_CONTROLENO2" id="UHLOG_CONTROLENO2"></input></div></td>
	</tr>
      <tr> 
      <td width="" colspan="2" align="left" style=""><div align="left"><input name="area_name" id="area_name" type="hidden" size="30" maxlength="50" readonly="readonly"/></div></td>
     </tr>
      <!-- <tr>      <td width="" height="" align="right" style=""><div align="right">Emp Id</div></td> -->
     <!--  <td width="" colspan="2" align="left"><div align="left"><input name="huma_id" type="text" id="huma_id" style="border-color:#0099FF; width: 264px" onkeyup="allowupdate();" maxlength="57" onfocus="addSuggestion2('huma_id','huma_id',BSFLUNIT_UCODE.value);"></div></td>  -->
      <td width="" colspan="2" align="left"><div align="left"><input name="huma_id" type="hidden" size="40" id="huma_id" style="border-color:#0099FF;" onkeyup="allowupdate();"  maxlength="57" onfocus="addSuggestion2('lsrcode','huma_id',BSFLUNIT_UCODE.value);"></div></td>
     
      <tr>      <td width="" height="" align="right" style=""><div align="right">Name of StateHead</div></td>
      
     <!--  <td width="" colspan="2" align="left"><div align="left"><input name="huma_id" type="text" id="huma_id" style="border-color:#0099FF; width: 264px" onkeyup="allowupdate();" maxlength="57" onfocus="addSuggestion2('huma_id','huma_id',BSFLUNIT_UCODE.value);"></div></td>  -->
      <td width="" colspan="2" align="left"><div align="left"><input name="name_sh" type="text" size="40" id="name_sh" style="border-color:#0099FF;" onkeyup="allowupdate();"  maxlength="57" onfocus="addSuggestion2('lsrcode','huma_id',BSFLUNIT_UCODE.value);"></div></td>
      </tr>
      <tr>      <td width="" height="" align="right" style=""><div align="right">Region</div></td>
      
     <!--  <td width="" colspan="2" align="left"><div align="left"><input name="huma_id" type="text" id="huma_id" style="border-color:#0099FF; width: 264px" onkeyup="allowupdate();" maxlength="57" onfocus="addSuggestion2('huma_id','huma_id',BSFLUNIT_UCODE.value);"></div></td>  -->
      <td width="" colspan="2" align="left"><div align="left"><input name="reg" type="text" size="40" id="reg" style="border-color:#0099FF;" onkeyup="allowupdate();"  maxlength="57" onfocus="addSuggestion2('lsrcode','huma_id',BSFLUNIT_UCODE.value);"></div></td>
      </tr>
      <tr>
            ...
            <td colspan="4" height="100%" valign="top" align="center"><div align="left"><b> <!--WAR Room Data  Entry--></b></div>
              <table id="tableID" width="100%" style="border:1px solid #708090;;width: 100%" ><!--  -->
                  ...
                  <!-- <tr bgcolor="back03.gif" style="opacity: 0.65; filter: alpha(opacity=65);"> -->
                    ...
                    <th width="30%" scope="col" class="style21" bgcolor="#E6E6FA" style="height: 16px; width: 420px" style="width: 303px; "><div align="center">Name Of The Unit Visited</div></th>
					...
					<th width="30%" scope="col" class="style21" bgcolor="#E6E6FA" style="height: 16px; width: 420px" style="width: 303px; "><div align="center">Name Of The LSR/UH Visited with</div></th>                    
					...
					<th width="16%" class="style21" scope="col" bgcolor="#E6E6FA" style="height: 16px; width: 164px"><div align="center">Remarks</div></th>
...
<td style="" valign="top"><div align="center"><input name="war_day" type="text" id="war_day" onkeyup="allowupdate();" maxlength="10" size="10" readonly="readonly"/></div></td>
...
<td valign="top"><div align="center"><input name="unit_visited"  type="text"   id="unit_visited"  style="width:90%" size="50" onkeyup="allowupdate();" maxlength="10" onfocus="addSuggestion('BSFLUNIT_UCODE_roles','unit_visited');"/></div></td>

<!--<td><div align="center"><input name="lsrname" type="text" id="lsrname"  style="border-color:#0099FF; width:95%" onkeyup="allowupdate();" onfocus="addSuggestion('lsrname','lsrname');"maxlength="57"  /></div></td>
 ...
 --> 					<!-- <td><div align="center"><input name="ACTIVITY_ID" type="text" id="ACTIVITY_ID"  style="border-color:#0099FF; width:95%" onkeyup="allowupdate();" maxlength="57" onfocus="showEmpn2('lsrname.ACTIVITY_ID',this,this.value);"/></div></td> -->
 					<td style="width: 395px; " align="left" valign="top">
 									...
 									<div id='TextBoxesGroup' align="left" style="width: 241px; ">
									...
									<input type="text" name="textbox1" id="textbox1" size="18" style="border-color:#0099FF;" onkeyup="allowupdate();" maxlength="65" onfocus="addSuggestion2('VCODE','textbox1',unit_visited.value);"></div>	
										</td>
					<td valign="top"><div align="center"><input name="visited_with" type="text" id="visited_with" style="width:90%" size="50" onkeyup="allowupdate();" onfocus="showEmpn2('NAME',this,this.value);" maxlength="10"/></div></td>
					<td valign="top"><div align="center"><input name="war_cust_count" type="text" id="war_cust_count" style="width:90%" size="5" onkeyup="allowupdate();" maxlength="3"/></div></td>
					...
					<td ><div align="center"><textarea name="observations" id="observations" style="width:95%" rows="2" onkeyup="EnforceMaximumLength(this,300); allowupdate();" onblur="EnforceMaximumLength(this,300);"></textarea></div></td>
                ...
                <table id="msg2" style="visibility:hidden; position:absolute;" align="">
                  ...
                  <td   width="" align="left" >&nbsp;&nbsp;
                    <input type="button" id="exct" name="exct" class="groovybutton" value="&euro;"    title="" onmouseover="goLite(this.form.name,this.name); showmenu('msg');"
   						onmouseout="goDim(this.form.name,this.name); hidemenu('msg');" onclick="showEmpn('actual_pl',BSFLUNIT_UCODE.value,huma_id.value,war_date.value);"/>
