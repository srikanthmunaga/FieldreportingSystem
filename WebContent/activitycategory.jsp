		...
		$("#form1").submit(function(){sessioncheck();//alert("hey inside the submit function");
		  if(document.form1.onSubmit==validateForm()) 
			...
			//alert("Before executing actcat");
			   ...
			   var program='acmstru'; //alert("hey the program is="+program);	
			   	$.ajax({ 
				   	url: program,
		          	data: allElements,
		          	type: 'post',
...
var showdata = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,''); //removes the starting & ending spaces spaces
	...
	if(star=='')
	 { 
		 ...
		 document.getElementById("cat_id1").value ='';
	 }
	...
	{ 
	 ...
	 //msr
     document.getElementById("cat_id1").readOnly =false;
	 nextprevious("first");
	}//esle
//  } 
}//stateChangedn() 
...
{ 
  ...
  { 
   ...
   document.getElementById("cat_id1").value =star[x][0];
...
//Everything from disable() exceptiong button code,just for primary key enable(replace true by false) and editable(add readOnly=false)
 ...
 document.form1.cat_id1.disabled=false;//3 lines, newly added code for the Query functionality
 document.form1.cat_id1.readOnly=false;
 document.form1.cat_id1.style.backgroundColor='skyblue';
...
//var inputs = f.getElementsByTagName("select");
 ...
 document.form1.cat_id1.readOnly=true;//if it was made editable in queryfunction
...
xmlHttp=GetXmlHttpObject();
 //alert("hey got the boject="+xmlHttp);
   if(xmlHttp==null)
  	  return;
    var url="getuserupdate";
    url=url+"?decide=cat_id1&key1=&key2=";
	...
	document.getElementById("cat_id1").value=id;//alert("hey the assignment to the weekoff_id field with its value is over");
 //starting automatic id generation code when click on new button----------------------------------------------------------------- 
					...
					<td width="620" height="250"><fieldset
							...
							<table bgcolor="" width="100%" height="91%" border="0"
								...
								<tr>
									...
									<td  align="left">
											<div align="left">
												<input name="cat_id1" id="cat_id1" type="text" size="55" maxlength="55" onFocus="addSuggestion('ACTCAT_ID','cat_id1');" readonly="readonly"/>(Ex: AC001)<!-- onfocus="addSuggestion('cat_id2','cat_id');" -->
                  ...
                  <tr>
                    <td >Execute</td>
													...
													<td width="" align="left">&nbsp;&nbsp; <input
														...
														onclick="showEmpn('comp_mstr11',cat_id1.value,'');" /> <br />
														<table id="msg"
