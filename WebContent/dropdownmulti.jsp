...
<title>Unit Indent Master</title>
...
<%//getting BSFLUNIT_UCODE to assign in BSFLUNIT_UCODE field automatically in enable() function
  ...
  String sql="select s_id||'-'||s_name from stationary_mstr";
...
function showEmps(decide,key1,key2)
{//alert("hety inside the showEmp");
 sessioncheck();
xmlHttp=GetXmlHttpObject();
 ...
 xmlHttp.send(null);//alert("the ajax request made ");
	...
	var strar = showdata.split("//////");
	 ...
	 if(strar=='')//if the entered busi_id is not exist in the DB then this if code runs
	 {
		 ...
		 // document.getElementById("o_stock").value='';
		 //document.getElementById("comp_id").value =" ";
	 }
	 else if(strar.length>0)
	 {
		document.form1.sta_name.value=strar[0];
		//document.form1.o_stock.value=strar[1];
	 }
 } 
//else
	//if(key1>=4)
...
function selectionchange()
{
    ...
    document.getElementById('sta_name').value = str;
}
...
{ 
   ...
   if (!date.test(document.form1.date_oi.value)) {           //code for date validation
		alert("Please select the date of indent date");
		document.form1.date_oi.focus();
		return false;
	}// alert("hi1");
	if (document.form1.date_oi.value.replace(/^\s+/, '').replace(/\s+$/, '') != "") {
		var vnvdate = (document.form1.date_oi.value).split("-");
		var validformat = /^\d{2}-\d{2}-\d{4}$/;
		//var returnval = false;// validformat
		if (!validformat.test(document.form1.date_oi.value)) {
			alert("Please enter the date of indent correct format");
			document.form1.date_oi.focus();
			return false;
		...
		if ((dayobj.getMonth() + 1 != monthfield)
			...
			alert("Invalid month or date found in date of indent");
			document.form1.date_oi.value="";
			document.form1.date_oi.focus();
			return false;
		}
		// Date comparision code starts here
		var date_of_i = (document.form1.date_oi.value).split("-");
		var date_of_ind = new Date(date_of_i[2], date_of_i[1] - 1,date_of_i[0]); // alert("hey
																			// to
																			// FRS_date
																			// date
																			// is
																			// ="+FRS_date);
		if (date_of_ind > sysdate) {
			alert("date of indent should not be grater than the System date");
			document.form1.date_oi.value="";
			document.form1.date_oi.focus();
			return false;
		}
	...
	if (document.getElementsByName("c_stock")[0].value == "") {
		alert("Please enter the closing stock");
		document.getElementsByName("c_stock")[0].focus();
		document.getElementsByName("c_stock")[0].style.background = "#fffacd";
		return false;
	}

	if (isNaN(document.getElementsByName("c_stock")[0].value)) {
		alert("closing stock must be a numeric value");
		document.getElementsByName("c_stock")[0].value = "";
		document.getElementsByName("c_stock")[0].focus();
		document.getElementsByName("c_stock")[0].style.background = "#fffacd";
		return false;
	}
	
	if (document.getElementsByName("n_stock")[0].value == "") {
		alert("Please enter the new stock");
		document.getElementsByName("n_stock")[0].focus();
		document.getElementsByName("n_stock")[0].style.background = "#fffacd";
		return false;
	}

	if (isNaN(document.getElementsByName("n_stock")[0].value)) {
		alert("new stock must be a numeric value");
		document.getElementsByName("n_stock")[0].value = "";
		document.getElementsByName("n_stock")[0].focus();
		document.getElementsByName("n_stock")[0].style.background = "#fffacd";
		return false;
	}
		  ...
		  if(document.form1.onSubmit==validateForm()) 
			...
			if(document.form1.save.style.display=='inline')
			  program='unit';	
			   ...
			   program='UnitUpdate'; //alert("hey the program is="+program);	
			   	$.ajax({ 
				   	url: program,
		          	data: allElements,
		          	type: 'post',
...
var showdata = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,''); //removes the starting & ending spaces spaces
	...
	// goDim("form1","exct");
     ...
     //document.getElementById("bsflunit_code").readOnly =true;
     goDim("form1","exct");
	document.getElementById("msg").style.visibility="hidden";
	 document.getElementById("exct").disabled=true;
...
{ 
 ...
 // { 
  ...
  var s= document.getElementById("bsflunit_ucode").value =star[x][0];
  alert(s);
  document.getElementById("unit_name").value =star[x][1];
  document.getElementById("st_id").value =star[x][2];
  document.getElementById("sta_name").value =star[x][3];
  document.getElementById("date_oi").value =star[x][4];
 // document.getElementById("o_stock").value =star[x][5];
  document.getElementById("c_stock").value =star[x][5];
  document.getElementById("n_stock").value =star[x][6];
  document.getElementById("in_by").value =star[x][7];
  document.getElementById("p_by").value =star[x][8];
...
<body bottommargin="100%" marginheight="100%" rightmargin="100%"
	...
	onLoad="disable();">
	<script language="javascript" type="text/javascript">var username_js="<%=(String) request.getSession()
						.getAttribute("username")%>";</script>
	<script language="javascript" type="text/javascript">var unitname_js="<%=(String) request.getSession()
						.getAttribute("unitname")%>";</script>
	<script language="javascript" type="text/javascript">var userrole_js="<%=(String) request.getSession()
						.getAttribute("userrole")%>";</script>
	<script language="javascript
		
	script">var userrole_js="<%=(String) request.getSession()
								...
								<td>
									<h4>
										<b><font color="white">Bhartiya Samruddhi Finance
												Limited</font></b>
									</h4> <br> <strong><font color="white">Field
											Reporting System</font></strong>
								</td>

								<td align="right" valign="bottom"><b> <font
										color="white" style="font-size: 11px"> <i><%=((HttpServletRequest) request).getSession()
						.getAttribute("username").toString()%></i>
											Logged in from <i><%=((HttpServletRequest) request).getSession()
						.getAttribute("unitname").toString()%></i>
											Unit
									</font>
								</b></td>
					...
					<table width="100%">
						<tr>
							...
							<td width="14%" onMouseOver="showmenu('Masters')"
								...
								<div align="center">
									<a href="Chome.jsp" class="style21" id="aMasters"> <u>M</u>asters
									</a><br />
								</div>
								<table class="menu" id="Masters" width="182" border="0"
    								...
    								<tr>
										<td class="menu"><a href="mchange.jsp">Change pwd</a></td>
									</tr>
   ...
   <tr><td class="menu"><a href="issuedstock.jsp">Issued Stock</a></td></tr>
							...
							<td width="17%" onMouseOver="showmenu('Reports')"
								...
								<div align="center">
									<a href="Chome.jsp" class="style21" id="aReports"> <u>R</u>eports
									</a><br />
								</div>
								<table class="menu" id="Reports" width="220" border="0"
									bordercolor="blue">
									<tr>
															<td class="menu"><a href="Unitindentrep.jsp">Unit Indent Report</a></td>
														</tr>
														<tr>
							...
							<td width="14%" onMouseOver="showmenu('Logoff')"
								...
								<div align="center">
									<a href="Chome.jsp" class="style21" id="aLogoff"> <u>L</u>ogoff
									</a><br />
								</div>
							...
							<td width="41%" class="bg_menu style21" align="right">Indent Management
								Module</td>
 	...
 	String role = (String) ((HttpServletRequest) request)
 		...
 		if ((role.equals("admin")) || (role.equals("audit"))||(role.equals("unithead")))// ||role.equals("areahead") || role.equals("unithead"))
 		{
 %>
					...
					<form id="form1" name="form1" method="post">
						...
						<table cellpadding="1" cellspacing="1">
							<tr>
								<!--  <td width="380" align="left"><b><!-- <a href="masters.jsp">Back</a> </b></td>-->
								<br>
							...
							<tr>
								<td width="606" height="250">
									<fieldset style="background-color:">
										<legend class="formTitle"> Unit Indent Master</legend>
										<table bgcolor="" width="100%" height="91%" border="0"
											align="center" bordercolor="#000000">
											<tr>
												<td height="44" colspan="4" align="right">
													<div align="right">
											...
											<tr>
												<td width="24%" align="right"><div align="right">Unit
														Code</div></td>
												<td><div align="left">
											...
											<tr style="width: 599px; ">
												...
												<td align="left" style="width: 256px; ">
													<div align="left">
														<select name="st_id" id="st_id" onblur="showEmps('s_id.s_name',st_id.value,'');"> 
														
        													<%  while(svrs.next()){ %>
            												<option><%= svrs.getString(1)%></option>
        														<% } %>
        													</select>
        													
											       </div>
												</td>
												<td align="left" style="width: 95px; ">Closing Stock
											</td>
								<td><input type="text" name="stock" id="stock"/>
											...
											<tr>
												...
												<td align="left">
													    <div align=left></div>
														<input name="sta_name" id="sta_name" type="text" placeholder="" autofocus="autofocus" readonly/>
       ...
       showCalender(this, 'date_oi');">
											...
											<tr>
												<td height="24" align="right"><div align="right">Closing
														Stock</div></td>
												<td align="left">
													<div align="left">
														<input name="c_stock" id="c_stock" type="text"
															size="20" maxlength="20"  onKeyUp="allowupdate();"/> 
													</div>
												</td>
											</tr>
											<tr>
												<td height="24" align="right"><div align="right">Stock Indent Now
													</div></td>
												<td align="left">
													<div align="left">
														<input name="n_stock" id="n_stock" type="text"
															size="20" maxlength="20" onKeyUp="allowupdate();"
															/>
													</div>
												</td>
											...
											<tr>
												<td colspan="4" align="center" valign="bottom" height="61">
													<div align="center">
														<table>
															<tr>
																<td><script type="text/javascript"
																		src="JS/Bbuttons.js"></script> <br />
																	<table id="msg2"
																...
																<td width="" align="left">&nbsp;&nbsp; <input
																	...
																	onclick="showEmpn('unit_inde',bsflunit_ucode.value,'');" /> <br />
																	<table id="msg"
																		style="visibility: hidden; position: absolute;">
																		<tr>
																			<td>Execute</td>
					...
					</form> <!---------------- Code for FRS theme FRS_Theme_bottom.js starts here  ------------------------->
...
<br>
<br>
<div align="center" class="style22">Sorry,NO Rights to create new
	Indent</div>
<br>
<br>
<br>
