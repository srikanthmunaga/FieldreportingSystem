...
//alert("hey url is made");
 ...
 xmlHttp.send(null);//alert("the ajax request made ");
	...
	var strar = showdata.split("::::::");
	 ...
	 {
		document.form1.comp_id.value=strar[0];
	 }//else
	}//if(key1>=4)
	else //if(key1<4)
	{
...
{ //alert("hey inside the validateForm() function");
  ...
  var compid=new RegExp("C[0-9]{3}"); //(compid.test(document.form1.a_name.value))
   if((!patt1.test(document.form1.a_name.value)) || (reason.test(document.form1.a_name.value)) || (compid.test(document.form1.a_name.value)))
    {
 ...
 //alert("hey end of the validateFom function");
  var compid=new RegExp("C[0-9]{3}"); //(compid.test(document.form1.a_name.value))
   if((!patt1.test(document.form1.cat_id1.value)) || (reason.test(document.form1.a_name.value)) || (compid.test(document.form1.a_name.value)))
    {
      alert("please select Category ID");
      document.form1.cat_id1.focus();
      return false;
    }//a_name
		  ...
		  if(document.form1.onSubmit==validateForm()) 
			...
			if(document.form1.save.style.display=='inline')
			  var program='amstr';	
			   ...
			   var program='amstru'; //alert("hey the program is="+program);	
			   	$.ajax({ 
				   	url: program,
		          	data: allElements,
		          	type: 'post',
...
var showdata = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,''); //removes the starting & ending spaces spaces
	...
	if(star=='')
	 { 
		 ...
		 document.getElementById("a_id1").value ='';
	 }
	...
	{ 
	 ...
	 //msr
     document.getElementById("a_id1").readOnly =false;
	 nextprevious("first");
	}//esle
//  } 
}//stateChangedn() 
...
{ 
  ...
  { 
   ...
   document.getElementById("a_id1").value =star[x][0];
  document.getElementById("a_name").value =star[x][1];
  document.getElementById("cat_id1").value =star[x][2];
 }//show()
...
//Everything from disable() exceptiong button code,just for primary key enable(replace true by false) and editable(add readOnly=false)
 ...
 document.form1.a_id1.disabled=false;//3 lines, newly added code for the Query functionality
 document.form1.a_id1.readOnly=false;
 document.form1.a_id1.style.backgroundColor='skyblue';
 document.form1.a_id1.focus();
//--------------------------------------Common (buttons) code
...
//var inputs = f.getElementsByTagName("select");
 ...
 document.form1.a_id1.readOnly=true;//if it was made editable in queryfunction
...
xmlHttp=GetXmlHttpObject();
 //alert("hey got the boject="+xmlHttp);
   if(xmlHttp==null)
  	  return;
    var url="getuserupdate";
    url=url+"?decide=a_id1&key1=&key2=";
    xmlHttp.open("GET",url,false);//xmlHttp.open("GET",url,true);
    xmlHttp.send(null);//alert("the ajax request made ");
	var id = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,'');//alert("hey goy the show data is:"+id);
	document.getElementById("a_id1").value=id;//alert("hey the assignment to the weekoff_id field with its value is over");
 //starting automatic id generation code when click on new button----------------------------------------------------------------- 
			...
			<table width="640" align="center">
				<tr>
					<td width="606" height="250"><fieldset
							...
							<legend class="formTitle">Activity Master</legend>
							<table bgcolor="" width="100%" height="91%" border="0"
						    	...
						    	<tr>
									...
									<td width="76%" align="left">
											<div align="left">
												<input name="cat_id1" id="cat_id1" type="text" size="55"
													maxlength="55" onFocus="addSuggestion('ACTCAT_ID1','cat_id1');" onKeyUp="allowupdate();"/><!--onfocus="addSuggestion('a_id2','cat_id');"  -->
											</div>
								</td>
								</tr>
								<tr>
									<td width="24%" align="right"><div
											align="right">Activity ID</div></td>
									<td width="76%" align="left">
											<div align="left">
													<input name="a_id1" id="a_id1" type="text" size="55"
													onfocus="addSuggestion('a_id2','a_id1');"
													maxlength="55" readonly="readonly"/>(Ex: A001)
											</div>
								</td>
								</tr>
								...
								<tr>
									<td height="24" align="right"><div
											align="right" >Activity Name </div></td>
									<td align="left">
										<div align="left">
											<input name="a_name" id="a_name" type="text" size="55"
												maxlength="60" onKeyUp="allowupdate();"
												onfocus="addSuggestion('a_id2','a_name');"
												/><!-- class="masterInput"  onfocus="addSuggestion('a_id2','a_name');" -->
										</div>
									</td>
								</tr>
								<tr>
                  ...
                  <tr>
                    <td >Execute</td>
													...
													<td width="" align="left">&nbsp;&nbsp; <input
														...
														onclick="showEmpn('act_mstr1',a_id1.value,'');" /> <br />
														<table id="msg"
...
Sorry,NO Rights to create new Activity
</div>
