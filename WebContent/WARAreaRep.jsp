...
<title>WAR Room Region Report</title>
<% 
				...
				<tr>
					<td height="">
						<fieldset style="background-color:">
							...
							<legend class="formTitle">WAR Room Region Report</legend>
							<table bgcolor="" width="100%" height="91%" border="0"
								...
								<tr>
									...
									<td><input type="hidden" name="report_name"
										id="report_name" value="WARAreaRep" /></td>
    ...
    con= dbConn.getConnection();
	  ...
	  { if(rs.next())//query to get all employees information,just like superuser
	    ...
	    */
		...
		if((role.equals("admin"))||(role.equals("audit")))// || role.equals("areahead") || role.equals("unithead"))
		ps2 = con.prepareStatement("select distinct AREA_ID,AREA_NAME,row_number() over(order by AREA_ID) as sno from AREA_MSTR order by AREA_ID");
		else if(role.equals("areahead"))
		ps2 = con.prepareStatement("select distinct AREA_ID,AREA_NAME,row_number() over(order by AREA_ID) as sno from AREA_MSTR where huma_id='"+huma_id+"' order by AREA_ID");
	    else
	    ps2 = con.prepareStatement("select distinct AREA_ID,AREA_NAME,row_number() over(order by AREA_ID) as sno from AREA_MSTR where huma_id='' order by AREA_ID");
	  ...
	  while(rs2.next())
		{
		  %>
                <tr>
                  <td><div align="center"><%=rs2.getString(3)%>&nbsp;</div></td>
