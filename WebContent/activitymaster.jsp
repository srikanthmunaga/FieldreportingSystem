...
{ //alert("hey inside the validateForm() function");
 ...
 //alert("hey end of the validateFom function");
  var compid=new RegExp("C[0-9]{3}"); //(compid.test(document.form1.a_name.value))
   if((!patt1.test(document.form1.cat_id.value)) || (reason.test(document.form1.a_name.value)) || (compid.test(document.form1.a_name.value)))
    {
		...
		$("#form1").submit(function(){sessioncheck();//alert("hey inside the submit function");
		  if(document.form1.onSubmit==validateForm()) 
			...
			if(document.form1.save.style.display=='inline')
			  var program='activity';	
			else  
			   var program='actvityupdate'; //alert("hey the program is="+program);	
			   	$.ajax({ 
				   	url: program,
		          	data: allElements,
		          	type: 'post',
...
var showdata = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,''); //removes the starting & ending spaces spaces
	...
	{ 
	 ...
	 //msr
     document.getElementById("a_id").readOnly =false;
	 nextprevious("first");
	}//esle
//  } 
}//stateChangedn() 
...
{ 
  ...
  { 
   ...
   document.getElementById("a_id").value =star[x][0];
  document.getElementById("a_name").value =star[x][1];
  document.getElementById("cat_id").value =star[x][2];
 }//show()
...
//Everything from disable() exceptiong button code,just for primary key enable(replace true by false) and editable(add readOnly=false)
 ...
 document.form1.a_id.disabled=false;//3 lines, newly added code for the Query functionality
 document.form1.a_id.readOnly=false;
 document.form1.a_id.style.backgroundColor='skyblue';
 document.form1.a_id.focus();
//--------------------------------------Common (buttons) code
...
xmlHttp=GetXmlHttpObject();
 //alert("hey got the boject="+xmlHttp);
   if(xmlHttp==null)
  	  return;
    var url="getuserupdate";
    url=url+"?decide=a_id&key1=&key2=";
    xmlHttp.open("GET",url,false);//xmlHttp.open("GET",url,true);
    xmlHttp.send(null);//alert("the ajax request made ");
	var id = xmlHttp.responseText.replace(/^\s+/,'').replace(/\s+$/,'');//alert("hey goy the show data is:"+id);
	document.getElementById("a_id").value=id;//alert("hey the assignment to the weekoff_id field with its value is over");
 //starting automatic id generation code when click on new button----------------------------------------------------------------- 
							...
							<table bgcolor="" width="100%" height="91%" border="0"
						    	...
						    	<tr>
									...
									<td width="76%" align="left">
											<div align="left">
												<input name="cat_id" id="cat_id" type="text" size="55"
													maxlength="55" onFocus="addSuggestion('ACTCAT_ID','cat_id');" onKeyUp="allowupdate();"/><!--onfocus="addSuggestion('a_id2','cat_id');"  -->
											</div>
								</td>
								</tr>
								<tr>
									...
									<td width="76%" align="left">
											<div align="left">
													<input name="a_id" id="a_id" type="text" size="55"
													onfocus="addSuggestion('a_id2','a_id');"
                  ...
                  <tr>
                    <td >Execute</td>
													...
													<td width="" align="left">&nbsp;&nbsp; <input
														...
														onclick="showEmpn('act_mstr',a_id.value,'');" /> <br />
														<table id="msg"
