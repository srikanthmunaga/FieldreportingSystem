...
<title>Field Staff Day wise Detailed Rep</title>
<% 
				...
				<tr>
					<td height="">
						<fieldset style="background-color:">
							...
							<legend class="formTitle">Field Staff Day wise Detailed Report</legend>
							<table bgcolor="" width="100%" height="91%" border="0"
								...
								<tr>
									...
									<td><input type="hidden" name="report_name"
										id="report_name" value="daywiselsrsummary" /></td>
    ...
    con= dbConn.getConnection();
	  ...
	  /*
	   if((role.equals("admin"))||(role.equals("audit")))// || role.equals("areahead") || role.equals("unithead"))
		//ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) order by huma_id");
		
	   else if(role.equals("areahead"))
	   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where area_id in (select area_id from area_mstr where huma_id='"+huma_id+"')) order by huma_id");
	   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where area_id in (select area_id from area_mstr where huma_id='"+huma_id+"')) order by huma_id");
	   else if(role.equals("unithead"))
	   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where  huma_id='"+huma_id+"') order by huma_id");
	   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where  huma_id='"+huma_id+"') order by huma_id");
	   else
		   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id ='"+huma_id+"'");
		   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE  huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id ='"+huma_id+"'");
	    //System.out.println("the query is going to be executed="+ps2);
	    */
	    if((role.equals("admin"))||(role.equals("audit")))// || role.equals("areahead") || role.equals("unithead"))
		//ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) order by huma_id");
		ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno, huma_status,(select grade_name from grade_mstr where grade_id=huma_designation) as huma_designation from huma_mstr WHERE upper(huma_status) LIKE '%ACTIVE%' and huma_DESIGNATION IN (select grade_id from grade_mstr where (GRADE_name like '%LSR%' or GRADE_name like '%FX%' or GRADE_name like '%FS%')) order by huma_id");
	   else if(role.equals("areahead"))
	   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where area_id in (select area_id from area_mstr where huma_id='"+huma_id+"')) order by huma_id");
	   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno, huma_status,(select grade_name from grade_mstr where grade_id=huma_designation) as huma_designation from huma_mstr WHERE upper(huma_status) LIKE '%ACTIVE%' and huma_DESIGNATION IN (select grade_id from grade_mstr where (GRADE_name like '%LSR%' or GRADE_name like '%FX%' or GRADE_name like '%FS%')) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where area_id in (select area_id from area_mstr where huma_id='"+huma_id+"')) order by huma_id");
	   else if(role.equals("unithead"))
	   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id in (select distinct huma_id from frs_recovery) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where  huma_id='"+huma_id+"') order by huma_id");
	   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno, huma_status,(select grade_name from grade_mstr where grade_id=huma_designation) as huma_designation from huma_mstr WHERE upper(huma_status) LIKE '%ACTIVE%' and huma_DESIGNATION IN (select grade_id from grade_mstr where (GRADE_name like '%LSR%' or GRADE_name like '%FX%' or GRADE_name like '%FS%')) and bsflunit_ucode in (select bsflunit_ucode from bsflunit_mstr where  huma_id='"+huma_id+"') order by huma_id");
	   else
		   //ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno from huma_mstr WHERE huma_DESIGNATION IN (select grade_id from grade_mstr where GRADE_level in ('Executive','Associate')) and huma_id not in (select distinct huma_id from frs_user where user_freeze='Y' and huma_id is not null) and huma_id ='"+huma_id+"'");
		   ps2 = con.prepareStatement("select huma_id,huma_fname,huma_lname,row_number() over(order by huma_id) as sno, huma_status,(select grade_name from grade_mstr where grade_id=huma_designation) as huma_designation from huma_mstr WHERE upper(huma_status) LIKE '%ACTIVE%' and huma_DESIGNATION IN (select grade_id from grade_mstr where (GRADE_name like '%LSR%' or GRADE_name like '%FX%' or GRADE_name like '%FS%')) and huma_id ='"+huma_id+"'");
	    //System.out.println("the query is going to be executed="+ps2);
	  ...
	  while(rs2.next())
		{
		  %>
                <tr>
                  ...
                  <td><div align="left"><%=rs2.getString(2)%>&nbsp;<%=rs2.getString(3)%></div></td>
                  <td><div align="left">
