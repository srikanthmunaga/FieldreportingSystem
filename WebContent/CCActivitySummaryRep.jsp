...
<title>CC Activity Summary Report</title>
<% 
				...
				<tr>
					<td height="">
						<fieldset style="background-color:">
							...
							<legend class="formTitle">CC Activity Summary Report</legend>
							<table bgcolor="" width="100%" height="91%" border="0"
								...
								<tr>
									...
									<td><input type="hidden" name="report_name"
										id="report_name" value="CCActivitySummaryRep" onMouseOver="goLite2(this);" onMouseOut="goDim2(this);"/></td>
								...
								<tr>
									...
									<td align="right"><input type="submit" name="format"
										...
										value="Generate XLS Report" onMouseOver="goLite2(this);" onMouseOut="goDim2(this);"/></td><td><input type="button" id="cancel" name="cancel" class="groovybutton" value="Cancel" title="" onMouseOver="goLite(this.form.name,this.name)"   onmouseout="goDim(this.form.name,this.name)" onClick="window.location='Chome.jsp'"/></td>

								</tr>
								<tr><td colspan="5">
								<table width="60%" align="center" border="1" style="border-style: outset" >
                <!--<select name="area_id" id="area_id" onchange="showEmp(this.value);">
    ...
    con= dbConn.getConnection();
	  ...
	  { if(rs.next())//query to get all employees information,just like superuser
		...
		*/
		...
		/* ps2 = con.prepareStatement("select distinct BSFLUNIT_UCODE,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno from BSFLUNIT_MSTR order by BSFLUNIT_UCODE"); */
		//ps2 = con.prepareStatement("select distinct BSFLUNIT_UCODE,BSFLUNIT_NAME,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno from huma_mstr WHERE DESIGNATION IN ('LSR','lsr','FX','fx','FS','fs')order by BSFLUNIT_UCODE");
		//By Sreenivas
		//ps2 = con.prepareStatement("select distinct BSFLUNIT_UCODE,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno from BSFLUNIT_MSTR order by BSFLUNIT_UCODE");
	   String role=request.getSession().getAttribute("userrole").toString();
		...
		ps2 = con.prepareStatement("select distinct BSFLUNIT_UCODE,BSFLUNIT_NAME,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno,(select area_name from area_mstr where area_id=(select area_id from bsflunit_mstr where BSFLUNIT_UCODE=b.BSFLUNIT_UCODE)) as area_name from BSFLUNIT_MSTR b order by BSFLUNIT_UCODE");
		else if(role.equals("areahead"))
		{
		//System.out.println("inside area head checked by Rajesh huma_id="+huma_id);
		ps2 = con.prepareStatement("select distinct BSFLUNIT_UCODE,BSFLUNIT_NAME,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno,(select area_name from area_mstr where area_id=(select area_id from bsflunit_mstr where BSFLUNIT_UCODE=b.BSFLUNIT_UCODE)) as area_name from BSFLUNIT_MSTR b where area_id in(select area_id from area_mstr where huma_id='"+huma_id+"') order by BSFLUNIT_UCODE");
		}
		...
		ps2 = con.prepareStatement("select BSFLUNIT_UCODE,BSFLUNIT_NAME,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno,(select area_name from area_mstr where area_id=(select area_id from bsflunit_mstr where BSFLUNIT_UCODE=b.BSFLUNIT_UCODE)) as area_name from BSFLUNIT_MSTR b where bsflunit_ucode in(select bsflunit_ucode from bsflunit_mstr where huma_id='"+huma_id+"') order by BSFLUNIT_UCODE");
		else
		ps2 = con.prepareStatement("select BSFLUNIT_UCODE,BSFLUNIT_NAME,BSFLUNIT_NAME,row_number() over(order by BSFLUNIT_UCODE) as sno,(select area_name from area_mstr where area_id=(select area_id from bsflunit_mstr where BSFLUNIT_UCODE=b.BSFLUNIT_UCODE)) as area_name from BSFLUNIT_MSTR b where bsflunit_ucode in(select bsflunit_ucode from bsflunit_mstr where huma_id='') order by BSFLUNIT_UCODE");	
	    rs2 = ps2.executeQuery();
	   %>
                <tr bgcolor="silver">
                  <th scope="col" ><div align="center" >Sno</div></th>
                  <th scope="col" ><div align="center" >Region Name</div></th>
                  <th scope="col" ><div align="center" >Unit Code</div></th>
